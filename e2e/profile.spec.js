...
test("profile page shows user info and badges", async ({ page }) => {
  ...
  await page.route("**/*supabase.co/**", (route) => {
    ...
    const FAKE_USER = { id: "test-user-1", email: "test@example.com", role: "authenticated" };
    ...
    test("profile page shows user info and badges", async ({ page }) => {
      ...
      await page.route("**/*supabase.co/**", (route) => {
        ...
        if (url.includes("/rest/v1/profiles")) {
          return route.fulfill({ status: 200, contentType: "application/json", body: JSON.stringify(DB_PROFILE) });
        }
      ...
      await expect(page.locator("text=RunnerSensei").first()).toBeVisible({ timeout: 10000 });

      const debug = await page.evaluate((key) => ({ session: localStorage.getItem(key), testMode: window.__TEST_MODE__, testProfile: window.__TEST_PROFILE__, allKeys: Object.keys(localStorage) }), SUPABASE_STORAGE_KEY);
      ...
      await page.screenshot({ path: path.join("test-results", "profile-debug.png") });
      await expect(page.locator("text=Profile").first()).toBeVisible({ timeout: 8000 });
      const bodyText = await page.evaluate(() => document.body.innerText);
      console.log("BODYTEXT:\n" + bodyText);
      await expect(page.locator('[aria-label="profile-initial"]').first()).toBeVisible({ timeout: 8000 });
      ...
      await expect(page.locator("text=Awarded when you log your first run.").first()).toBeVisible({ timeout: 5000 });
    });
